  ...
  reducers: {
    ...
    nextTrack: (state, action) => {
      const currentIndex = state.tracks.findIndex(
        (el) => el.id === state.trackId.id
      )

      if (action.payload === 'shuffle') {
        const nextTrack = Math.floor(Math.random() * state.tracks.length) // ищем рандомный трек
        state.trackId = state.tracks[nextTrack] // присваиваем его trackId
        return
      }
...
export const { setTrackId, setTracksRedux, nextTrack, prevTrack } =
  trackSlice.actions
